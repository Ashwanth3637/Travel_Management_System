function CustomerNavbar({ customer, handleLogout }) {
  return (
    <nav className="navbar mb-8 flex justify-between items-center flex-wrap gap-4">
      <div className="nav-logo flex items-center">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="mr-2">
          <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path>
          <circle cx="9" cy="7" r="4"></circle>
          <path d="M23 21v-2a4 4 0 0 0-3-3.87"></path>
          <path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
        </svg>
        TravelGo <span className="text-xs opacity-80 font-semibold ml-1.5 text-blue-400">AI Fleet</span>
      </div>
      <div className="nav-user flex items-center gap-3">
        <span className="badge badge-inprogress uppercase tracking-wide">RIDER</span>
        <span className="text-[15px] font-extrabold text-white tracking-wide">
          {customer ? customer.name : 'Rider'}
        </span>
      </div>
    </nav>
  );
}

export default CustomerNavbar;
